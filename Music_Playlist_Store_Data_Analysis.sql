--------------                 Music Playlist Store database Analysis          -----------

--- To hover between the differnet database tables------
...
SELECT billing_country,COUNT(invoice_id) AS max_invoices 
FROM invoice
...
--- Method 1: (Not optimised since multiple joins are used in a single statement, which leads our processor to take
---            longer time to process and ram ran out of space, but will give the correct answer)
...
---  to find which customer spent the most on this artist. For this query, you will need to use the Invoice, 
---  InvoiceLine, Track, Customer, Album, and Artist tables. Note, this one is tricky because the Total spent in the
---  Invoice table might not be on a single product, so you need to use the InvoiceLine table to find out how many 
...
WITH RECURSIVE
	sales_per_country AS(
		SELECT COUNT(*) AS max_purchases_genre, customer.country, genre.name, genre.genre_id
		FROM invoice_line
		JOIN invoice ON invoice.invoice_id = invoice_line.invoice_id
		JOIN customer ON customer.customer_id = invoice.customer_id
		JOIN track ON track.track_id = invoice_line.track_id
	...
	max_genre_per_country AS (SELECT MAX(max_purchases_genre) AS max_genre_number, country
...
WHERE sales_per_country.max_purchases_genre = max_genre_per_country.max_genre_number;
...
---   Steps to Solve:  Similar to the above question. There are two parts in question- first find the most spent on
---                    music for each country and second filter the data for respective customers.
